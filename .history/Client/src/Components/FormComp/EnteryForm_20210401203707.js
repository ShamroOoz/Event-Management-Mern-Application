...
const EnteryForm = ({ isSignup }) => {
  ...
  const [value, setvalue] = useState(SingupinitialValues);
  const [schema, setschema] = useState(SingupSchema);
  useEffect(() => {
    setvalue(isSignup ? SingupinitialValues : SignInSchema);
    setschema(isSignup ? SingupSchema : SignIinitialValues);
    return () => {
      setvalue(null);
      setschema(null);
    };
