...
const Form = () => {
  ...
  const onFailure = (response) => {
    console.log("login failed : currentUser:", response);
  };
  const { signIn } = useGoogleLogin({
    onSuccess,
    clientId:
      "564033717568-e5p23rhvcs4i6kffgsbci1d64r8hp6fn.apps.googleusercontent.com",
    onFailure,
    prompt,
  });

  useEffect(() => {
    setisSignup(pathname === "/singup" ? true : false);
  }, [pathname]);
